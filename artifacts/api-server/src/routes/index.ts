...
// NOTE: proxyRouter (generic + MovieBox + VidLink + AnimeDekho's shared /m3u8
// + /subtitle-proxy) is mounted first, so its route handlers win for any path
// also defined in the per-provider copies below. Those per-provider copies
// (animesalt/hindmovies/movies4u/kartoons) are full duplicates kept
// self-contained in each provider's folder per the folder-isolation
// convention — see replit.md. They're mounted too so each provider's block in
// stremio.ts can be pointed at its own copy independently in the future.
