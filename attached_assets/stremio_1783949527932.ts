...
router.get("/stream/:type/:id.json", async (req, res) => {
  ...
  try {
    ...
    if (id.startsWith("tt")) {
      ...
      // For movies, parts[1]/parts[2] are absent → leave undefined (not 1).
      // Defaulting to 1 was wrong: it caused fresh-stream URLs to carry
      // spurious ?sn=1&ep=1 for movies, and polluted the stream cache key.
    ...
    if (id.startsWith("tmdb:")) {
      ...
      const contentType = type as "movie" | "series";
      // Same fix: movies have no season/episode in their ID, default to undefined.
